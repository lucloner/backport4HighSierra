#!/bin/bash
# Build .NET 8 CoreCLR native components on macOS 10.13
set -e
...
echo "=== .NET 8 CoreCLR Build for macOS 10.13 ==="
echo "Source: $SRCROOT"
echo "Build:  $BUILDDIR"

# Step 1: Apply patches
echo "[1/5] Applying patches..."
for p in 01-deployment-target.patch 02-cryptokit-openssl-fallback.patch 03-swift-flags.patch 04-const-correctness.patch; do
  if [ -f "$p" ]; then
    echo "  Applying $p"
...
cmake "$SRCROOT/src/coreclr" \
  ...
  -DCMAKE_ASM_FLAGS="-mavx512f -mavx512bw -mavx512vl" \
  -DCMAKE_C_FLAGS="-mavx512f -mavx512bw -mavx512vl" \
...
mkdir -p "$OUTDIR/dylibs" "$OUTDIR/bin" "$OUTDIR/static-libs" "$OUTDIR/native-libs"
