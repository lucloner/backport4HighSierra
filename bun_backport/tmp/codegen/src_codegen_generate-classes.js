import { createRequire } from "node:module"; const __require = createRequire("file:////Volumes/Data/src/bun/src/codegen/");
...
function camelCase(string) {
  return string.split(/[\s_]/).map((e, i) => i ? e.charAt(0).toUpperCase() + e.slice(1).toLowerCase() : e.toLowerCase());
}
...
static const JSC::DOMJIT::Signature DOMJITSignatureFor${fnName}(${DOMJITName(fnName)}Wrapper,
  ${jsClassName}::info(),
  ${pure ? "JSC::DOMJIT::Effect::forPure()" : "JSC::DOMJIT::Effect::forReadWrite(JSC::DOMJIT::HeapRange::top(), JSC::DOMJIT::HeapRange::top())"},
...
function zigExportName(to, symbolName2, prop) {
  ...
  if (getter && !to.get(getter)) {
    to.set(getter, exportNames.getter = symbolName2(getter));
  }
  if (setter && !to.get(setter)) {
    to.set(setter, exportNames.setter = symbolName2(setter));
  }
  if (fn && !to.get(fn)) {
    if (DOMJIT) {
      to.set(DOMJITName(fn), exportNames.DOMJIT = symbolName2(DOMJITName(fn)));
    }
    to.set(fn, exportNames.fn = symbolName2(fn));
  }
  return exportNames;
}
function propRow(symbolName2, typeName, name, prop, isWrapped = true, defaultPropertyAttributes, supportsObjectCreate = false, disableDom) {
  var {
    defaultValue,
    getter,
    setter,
    ...
    builtin,
    writable = false
  } = defaultPropertyAttributes ? Object.assign({}, defaultPropertyAttributes, prop) : prop;
  ...
  var symbol = symbolName2(typeName, name);
  if (isWrapped) {
    if (getter) {
    ...
    if (getter) {
      getter = symbolName2(typeName, getter);
    }
    if (setter || writable) {
      setter = symbolName2(typeName, setter);
    }
    if (fn) {
      fn = symbolName2(typeName, fn);
    }
  }
  if (builtin !== void 0) {
    if (typeof builtin !== "string") throw new Error('"builtin" should be string');
    return `
{ "${name}"_s, static_cast<unsigned>(JSC::PropertyAttribute::Builtin), NoIntrinsic, { HashTableValue::BuiltinGeneratorType, ${builtin}, ${length || 0} } }
`.trim();
  } else if (fn !== void 0) {
    if (DOMJIT) {
      return `
    ...
    return `
{ "${name}"_s, static_cast<unsigned>(JSC::PropertyAttribute::Function${extraPropertyAttributes}), NoIntrinsic, { HashTableValue::NativeFunctionType, ${fn}, ${length || 0} } }
`.trim();
...
function generateHashTable(nameToUse, symbolName2, typeName, obj, props = {}, wrapped) {
  const rows = [];
  let defaultPropertyAttributes = void 0;
...
function generateHashTableComment(nameToUse, symbolName2, obj, props = {}, wrapped) {
  const rows = [];
  let defaultPropertyAttributes = void 0;
  if ("enumerable" in obj) {
    defaultPropertyAttributes ||= {};
...
namespace WebCore {
static JSC::JSValue construct${symbolName2(name)}PropertyCallback(JSC::VM &vm, JSC::JSObject* initialThisObject);
}
    `;
    rows.push(`${name}  WebCore::construct${symbolName2(name)}PropertyCallback    PropertyCallback`);
  }
  if (rows.length === 0) {
    return "";
  }
...
`;
  ...
  if (obj.structuredClone) {
    externs += `extern JSC_CALLCONV void JSC_HOST_CALL_ATTRIBUTES ${symbolName(
      typeName,
...
`;
    externs += `extern JSC_CALLCONV JSC::EncodedJSValue JSC_HOST_CALL_ATTRIBUTES ${symbolName(
      typeName,
...
`;
  ...
  if (obj.finalize) {
    externs += `extern JSC_CALLCONV void JSC_HOST_CALL_ATTRIBUTES ${classSymbolName(typeName, "finalize")}(void*);
`;
  }
  if (obj.call) {
...
`;
  ...
  for (const name in protoFields) {
    ...
    if (protoFields[name].privateSymbol !== void 0) {
      ...
      specialSymbols += `
    this->putDirect(vm, WebCore::clientData(vm)->builtinNames().${privateSymbol}PrivateName(), JSFunction::create(vm, globalObject, ${protoFields[name].length || 0}, String("${observable_name}"_s), ${protoSymbolName(
        typeName,
        fn
      )}Callback, ImplementationVisibility::Private), PropertyAttribute::ReadOnly | PropertyAttribute::DontEnum | 0);`;
      continue;
    }
    if (protoFields[name].publicSymbol !== void 0) {
      const publicSymbol = protoFields[name].publicSymbol;
      const fn = protoFields[name].fn;
      if (!fn) throw Error(`(field: ${name}) public field needs 'fn' key `);
      const observable_name = protoFields[name].name ?? fn;
      specialSymbols += `
    this->putDirect(vm, WebCore::clientData(vm)->builtinNames().${publicSymbol}PublicName(), JSFunction::create(vm, globalObject, ${protoFields[name].length || 0}, String("${observable_name}"_s), ${protoSymbolName(
        typeName,
        fn
...
void ${proto}::finishCreation(JSC::VM& vm, JSC::JSGlobalObject* globalObject)
{
    ...
    ${Object.keys(protoFields).length > 0 ? `reifyStaticProperties(vm, ${className(typeName)}::info(), ${proto}TableValues, *this);` : ""}${specialSymbols}${staticPrototypeValues}
...
function generateConstructorImpl(typeName, obj) {
  ...
  const hashTable = Object.keys(fields).length > 0 ? generateHashTable(name, classSymbolName, typeName, obj, fields, false) : "";
...
${obj.call ? !obj.constructNeedsThis ? `
...
${obj.call ? `    RETURN_IF_EXCEPTION(scope, {});
  ${obj.estimatedSize ? `
...
function renderCallbacksHeader(typeName, callbacks) {
  const rows = [];
...
function renderCallbacksCppImpl(typeName, callbacks) {
  const rows = [];
  ...
  extern JSC_CALLCONV void ${symbolName(typeName, "_setAllCallbacks")}(JSC::EncodedJSValue encodedThisValue, ${Object.keys(
    callbacks
  ).map((_, i) => `JSC::EncodedJSValue encodedCallback${i}`).join(", ")}) {
    auto* thisObject = uncheckedDowncast<${className(typeName)}>(JSValue::decode(encodedThisValue));
    ${Object.keys(callbacks).map(
    (name, i) => `
...
function renderCallbacksZig(typeName, callbacks) {
  if (Object.keys(callbacks).length === 0) return "";
  var out = `
...
`;
  ...
  out = out.trim();
  out += `
  extern fn ${symbolName(typeName, "_setAllCallbacks")}(jsc.JSValue, ${Object.keys(callbacks).map((a, i) => `callback${i}: jsc.JSValue`).join(", ")}) callconv(jsc.conv) void;

  pub inline fn set(this: @This(), values: struct {
    ${Object.keys(callbacks).map((name, i) => `${camelCase(name)}: jsc.JSValue = .zero,`).join("\n")}
  }) void {
    ${symbolName(typeName, "_setAllCallbacks")}(this.instance, ${Object.keys(callbacks).map((name, i) => `values.${camelCase(name)}`).join(", ")},);
  }
  `;
  out += "\n};\n";
  out += `
...
function renderDecls(symbolName2, typeName, proto, supportsObjectCreate = false) {
  const rows = [];
  for (const name in proto) {
    if ("getter" in proto[name] || "accessor" in proto[name] && proto[name].getter) {
      externs += `extern JSC_CALLCONV JSC::EncodedJSValue JSC_HOST_CALL_ATTRIBUTES ${symbolName2(
        typeName,
...
`;
      rows.push(
        `
    JSC_DECLARE_CUSTOM_GETTER(${symbolName2(typeName, name)}GetterWrap);
    ${proto[name].writable ? `JSC_DECLARE_CUSTOM_SETTER(${symbolName2(typeName, name)}SetterWrap);` : ""}
    `.trim(),
        "\n"
      );
      if (supportsObjectCreate && !("setter" in proto[name])) {
        rows.push(`
static JSC_DECLARE_CUSTOM_SETTER(${symbolName2(typeName, name)}SetterWrap);
`);
      }
    }
    if ("setter" in proto[name] || "accessor" in proto[name] && proto[name].setter) {
      externs += `extern JSC_CALLCONV bool JSC_HOST_CALL_ATTRIBUTES ${symbolName2(typeName, proto[name].setter || proto[name].accessor.setter)}(void* ptr,${!!proto[name].this ? " JSC::EncodedJSValue thisValue, " : ""} JSC::JSGlobalObject* lexicalGlobalObject, JSC::EncodedJSValue value);
`;
      rows.push(
        `
      static JSC_DECLARE_CUSTOM_SETTER(${symbolName2(typeName, name)}SetterWrap);
...
`;
      rows.push(
        `
        JSC_DECLARE_HOST_FUNCTION(${symbolName2(typeName, name)}Callback);
        `.trim(),
        "\n"
      );
      if (proto[name].DOMJIT) {
        rows.push(
          DOMJITFunctionDeclaration(
            className(typeName),
            symbolName2(typeName, name),
            symbolName2(typeName, proto[name].fn),
            proto[name].DOMJIT
          ),
...
function renderStaticDecls(symbolName2, typeName, fields, supportsObjectCreate = false) {
  const rows = [];
  for (const name in fields) {
    if ("getter" in fields[name] || "accessor" in fields[name] && fields[name].getter) {
      externs += `extern JSC_CALLCONV JSC_DECLARE_CUSTOM_GETTER(${symbolName2(
        typeName,
...
`;
    ...
    if ("setter" in fields[name] || "accessor" in fields[name] && fields[name].setter) {
      externs += `extern JSC_CALLCONV JSC_DECLARE_CUSTOM_SETTER(${symbolName2(
        typeName,
...
`;
    ...
    if ("fn" in fields[name]) {
      externs += `extern JSC_CALLCONV JSC_DECLARE_HOST_FUNCTION(${symbolName2(typeName, fields[name].fn)}) SYSV_ABI;
`;
    }
  }
  return rows.join("\n");
}
function writeBarrier(symbolName2, typeName, name, cacheName) {
  return `

extern JSC_CALLCONV void ${symbolName2(typeName, name)}SetCachedValue(JSC::EncodedJSValue thisValue, JSC::JSGlobalObject *globalObject, JSC::EncodedJSValue value)
{
...
extern JSC_CALLCONV JSC::EncodedJSValue ${symbolName2(typeName, name)}GetCachedValue(JSC::EncodedJSValue thisValue)
{
  auto* thisObject = uncheckedDowncast<${className(typeName)}>(JSValue::decode(thisValue));
...
function renderFieldsImpl(symbolName2, typeName, obj, proto, cachedValues) {
  const rows = [];
    ...
    if ("cache" in proto[name] || proto[name]?.internal) {
      ...
      if ("cache" in proto[name]) {
        if (!supportsObjectCreate) {
          rows.push(
            `
JSC_DEFINE_CUSTOM_GETTER(${symbolName2(typeName, name)}GetterWrap, (JSGlobalObject * lexicalGlobalObject, EncodedJSValue encodedThisValue, PropertyName attributeName))
{
    auto& vm = JSC::getVM(lexicalGlobalObject);
    Zig::GlobalObject *globalObject = reinterpret_cast<Zig::GlobalObject*>(lexicalGlobalObject);
    auto throwScope = DECLARE_THROW_SCOPE(vm);
    ...
    JSC::JSValue result = JSC::JSValue::decode(
        ${symbolName2(typeName, proto[name].getter)}(thisObject->wrapped(),${proto[name].this ? " encodedThisValue, " : ""} globalObject)
    );
    ...
    RELEASE_AND_RETURN(throwScope, JSValue::encode(result));
          ...
          if (proto[name].writable) {
            rows.push(
              `
JSC_DEFINE_CUSTOM_SETTER(${symbolName2(typeName, name)}SetterWrap, (JSGlobalObject * lexicalGlobalObject, EncodedJSValue encodedThisValue, EncodedJSValue encodedValue, PropertyName attributeName))
{
    auto& vm = JSC::getVM(lexicalGlobalObject);
    auto throwScope = DECLARE_THROW_SCOPE(vm);
    ${className(typeName)}* thisObject = uncheckedDowncast<${className(typeName)}>(JSValue::decode(encodedThisValue));
    ...
    RELEASE_AND_RETURN(throwScope, true);
          ...
          rows.push(
            `
JSC_DEFINE_CUSTOM_GETTER(${symbolName2(typeName, name)}GetterWrap, (JSGlobalObject * globalObject, EncodedJSValue encodedThisValue, PropertyName attributeName))
{
    auto& vm = JSC::getVM(globalObject);
    auto throwScope = DECLARE_THROW_SCOPE(vm);
    ...
    JSC::JSValue result = JSC::JSValue::decode(
        ${symbolName2(typeName, proto[name].getter)}(thisObject->wrapped(),${proto[name].this ? " thisValue, " : ""} globalObject)
    );
    RETURN_IF_EXCEPTION(throwScope, {});
    thisObject->${cacheName}.set(vm, thisObject, result);
#if ASSERT_ENABLED
...
`.trim()
      ...
      rows.push(writeBarrier(symbolName2, typeName, name, cacheName));
...
JSC_DEFINE_CUSTOM_GETTER(${symbolName2(typeName, name)}GetterWrap, (JSGlobalObject * lexicalGlobalObject, EncodedJSValue encodedThisValue, PropertyName attributeName))
{
    ...
    ${className(typeName)}* thisObject = uncheckedDowncast<${className(typeName)}>(JSValue::decode(encodedThisValue));
    JSC::EnsureStillAliveScope thisArg = JSC::EnsureStillAliveScope(thisObject);
    JSC::EncodedJSValue result = ${symbolName2(typeName, proto[name].getter)}(thisObject->wrapped(),${!!proto[name].this ? " encodedThisValue, " : ""} globalObject);
    RETURN_IF_EXCEPTION(throwScope, {});
#if ASSERT_ENABLED
...
JSC_DEFINE_CUSTOM_SETTER(${symbolName2(typeName, name)}SetterWrap, (JSGlobalObject * lexicalGlobalObject, EncodedJSValue encodedThisValue, EncodedJSValue encodedValue, PropertyName attributeName))
{
    ...
    ${className(typeName)}* thisObject = uncheckedDowncast<${className(typeName)}>(JSValue::decode(encodedThisValue));
    JSC::EnsureStillAliveScope thisArg = JSC::EnsureStillAliveScope(thisObject);
    bool result = ${symbolName2(typeName, proto[name].setter || proto[name].accessor.setter)}(thisObject->wrapped(),${!!proto[name].this ? " encodedThisValue, " : ""} lexicalGlobalObject, encodedValue);
    RELEASE_AND_RETURN(throwScope, result);
}
`
      );
...
JSC_DEFINE_CUSTOM_SETTER(${symbolName2(typeName, name)}SetterWrap, (JSGlobalObject * lexicalGlobalObject, EncodedJSValue encodedThisValue, EncodedJSValue encodedValue, PropertyName attributeName))
{
  ...
  `
    ...
    if ("fn" in proto[name]) {
      ...
      const invalidThisBehavior = proto[name].invalidThisBehavior ?? 0 /* Throw */;
      rows.push(`
JSC_DEFINE_HOST_FUNCTION(${symbolName2(typeName, name)}Callback, (JSGlobalObject * lexicalGlobalObject, CallFrame* callFrame))
{
  auto& vm = JSC::getVM(lexicalGlobalObject);
  auto scope = DECLARE_THROW_SCOPE(vm);
  ...
  JSC::EnsureStillAliveScope thisArg = JSC::EnsureStillAliveScope(thisObject);
    ...
    JSC::EncodedJSValue result = ${symbolName2(typeName, fn)}(thisObject->wrapped(), lexicalGlobalObject, callFrame${proto[name].passThis ? ", JSValue::encode(thisObject)" : ""});
  ...
  return ${symbolName2(typeName, proto[name].fn)}(thisObject->wrapped(), lexicalGlobalObject, callFrame${proto[name].passThis ? ", JSValue::encode(thisObject)" : ""});
}
  ...
  if (cachedValues?.length) {
    for (const cacheName of cachedValues) {
      rows.push(writeBarrier(symbolName2, typeName, cacheName, "m_" + cacheName));
    }
  }
  return rows.map((a) => a.trim()).join("\n");
}
function allCachedValues(obj) {
...
`;
  ...
  const DECLARE_VISIT_CHILDREN = values.length || obj.estimatedSize || obj.valuesArray || Object.keys(callbacks).length || [...Object.values(klass), ...Object.values(proto)].find((a) => a.cache === true) ? "DECLARE_VISIT_CHILDREN;\n" : "";
  ...
  class ${name}${final ? " final" : ""} : public JSC::JSDestructibleObject {
    public:
        ...
        ${obj.noConstructor ? "" : `static JSObject* createConstructor(VM& vm, JSGlobalObject* globalObject, JSValue prototype)`};
        ...
        ${obj.valuesArray ? `const WTF::FixedVector<JSC::WriteBarrier<JSC::Unknown>>& values() const { return jsvalueArray; }
        ` : ""}
        ...
        ${obj.valuesArray && obj.values && obj.values.length > 0 ? `${name}(JSC::VM& vm, JSC::Structure* structure, void* sinkPtr${obj.values.map((v) => `, JSC::JSValue ${v}`).join("")})
        ...
        ${obj.valuesArray && obj.values && obj.values.length > 0 ? `${name}(JSC::VM& vm, JSC::Structure* structure, void* sinkPtr, WTF::FixedVector<JSC::WriteBarrier<JSC::Unknown>>&& jsvalueArray_${obj.values.map((v) => `, JSC::JSValue ${v}`).join("")})
        ...
        ${Object.entries(obj.custom ?? {}).map(([fieldName, field]) => {
...
function generateClassImpl(typeName, obj) {
  ...
  const name = className(typeName);
  let DEFINE_VISIT_CHILDREN_LIST = [...Object.entries(fields), ...Object.entries(proto)].filter(([name2, { cache = false }]) => cache === true).map(([name2]) => `visitor.append(thisObject->m_${name2});`).join("\n");
  for (const name2 in callbacks) {
    DEFINE_VISIT_CHILDREN_LIST += `
    visitor.appendHidden(thisObject->m_callback_${name2});`;
  }
  const values = (obj.values || []).map((val) => {
...
void ${name}::visitChildrenImpl(JSCell* cell, Visitor& visitor)
{
    ...
    ${estimatedSize ? `if (auto* ptr = thisObject->wrapped()) {
            auto size = ${symbolName(typeName, "estimatedSize")}(ptr);
...
`;
  ...
  if (hasPendingActivity) {
    externs += `extern JSC_CALLCONV bool JSC_HOST_CALL_ATTRIBUTES ${symbolName(typeName, "hasPendingActivity")}(void* ptr);
`;
    output += `
...
`;
  ...
  if (obj.hasOwnProperties()) {
    output += Object.entries(own).map(
      ([name2, getterName]) => `
static JSC::JSValue construct${symbolName(obj.name, name2)}PropertyCallback(JSC::VM &vm, JSC::JSObject* initialThisObject) {
    auto scope = DECLARE_THROW_SCOPE(vm);
...
${obj.valuesArray ? `${name}* ${name}::create(JSC::VM& vm, JSC::JSGlobalObject* globalObject, JSC::Structure* structure, void* ctx, WTF::FixedVector<JSC::WriteBarrier<JSC::Unknown>>&& jsvalueArray) {
  ${name}* ptr = new (NotNull, JSC::allocateCell<${name}>(vm)) ${name}(vm, structure, ctx, WTF::move(jsvalueArray));
  ptr->finishCreation(vm);
  return ptr;
}` : ""}

${obj.valuesArray && obj.values && obj.values.length > 0 ? `${name}* ${name}::create(JSC::VM& vm, JSC::JSGlobalObject* globalObject, JSC::Structure* structure, void* ctx${obj.values.map((v) => `, JSC::JSValue ${v}`).join("")}) {
  ${name}* ptr = new (NotNull, JSC::allocateCell<${name}>(vm)) ${name}(vm, structure, ctx${obj.values.map((v) => `, ${v}`).join("")});
  ptr->finishCreation(vm);
  return ptr;
}` : ""}

${obj.valuesArray && obj.values && obj.values.length > 0 ? `${name}* ${name}::create(JSC::VM& vm, JSC::JSGlobalObject* globalObject, JSC::Structure* structure, void* ctx, WTF::FixedVector<JSC::WriteBarrier<JSC::Unknown>>&& jsvalueArray${obj.values.map((v) => `, JSC::JSValue ${v}`).join("")}) {
...
void ${name}::analyzeHeap(JSCell* cell, HeapAnalyzer& analyzer)
{
    ...
    ${allCachedValues(obj).map(
    ([name2, cacheName]) => `
if (JSValue ${cacheName}Value = thisObject->${cacheName}.get()) {
  if (${cacheName}Value.isCell()) {
    const Identifier& id = Identifier::fromString(vm, "${name2}"_s);
...
${!obj.noConstructor ? `JSObject* ${name}::createConstructor(VM& vm, JSGlobalObject* globalObject, JSValue prototype)
{
...
${obj.valuesArray ? `extern JSC_CALLCONV JSC::EncodedJSValue JSC_HOST_CALL_ATTRIBUTES ${typeName}__createWithValues(Zig::GlobalObject* globalObject, void* ptr, void* markedArgumentBuffer) {
  auto &vm = globalObject->vm();
  JSC::Structure* structure = globalObject->${className(typeName)}Structure();
...
${obj.valuesArray && obj.values && obj.values.length > 0 ? `extern JSC_CALLCONV JSC::EncodedJSValue JSC_HOST_CALL_ATTRIBUTES ${typeName}__createWithInitialValues(Zig::GlobalObject* globalObject, void* ptr${obj.values.map((v) => `, JSC::EncodedJSValue ${v}`).join("")}) {
  auto &vm = globalObject->vm();
  JSC::Structure* structure = globalObject->${className(typeName)}Structure();
...
${obj.valuesArray && obj.values && obj.values.length > 0 ? `extern JSC_CALLCONV JSC::EncodedJSValue JSC_HOST_CALL_ATTRIBUTES ${typeName}__createWithValuesAndInitialValues(Zig::GlobalObject* globalObject, void* ptr, void* markedArgumentBuffer${obj.values.map((v) => `, JSC::EncodedJSValue ${v}`).join("")}) {
  auto &vm = globalObject->vm();
  JSC::Structure* structure = globalObject->${className(typeName)}Structure();
  auto* args = static_cast<JSC::MarkedArgumentBuffer*>(markedArgumentBuffer);
  WTF::FixedVector<JSC::WriteBarrier<JSC::Unknown>> jsvalueArray(args->size());
...
`;
function generateOwnProperties(typeName, symbolName2, obj, props = {}, wrapped) {
  lutTextFile += `
${generateHashTableComment(typeName, symbolName2, obj, props, wrapped)}
`;
}
...
function generateZig(typeName, {
  ...
  const cached_values_string = gc_fields.length > 0 ? `
      ...
      ${gc_fields.map(([name]) => `${name},`).join("\n")}
        ...
        pub fn get(comptime field: gc, thisValue: jsc.JSValue) ?jsc.JSValue {
          const value = switch (field) {
            ${gc_fields.map(([name]) => `    .${name} => ${protoSymbolName(typeName, name)}GetCachedValue(thisValue),`).join("\n        ")}
          };
        ...
        pub fn set(comptime field: gc, thisValue: jsc.JSValue, globalObject: *jsc.JSGlobalObject, value: jsc.JSValue) void {
          switch (field) {
            ${gc_fields.map(
    ([name]) => `    .${name} => ${protoSymbolName(typeName, name)}SetCachedValue(thisValue, globalObject, value),`
  ).join("\n")}
          }
        }
    };
  ` : "";
  const externs2 = gc_fields.map(
    ([name]) => `extern fn ${protoSymbolName(typeName, name)}SetCachedValue(jsc.JSValue, *jsc.JSGlobalObject, jsc.JSValue) callconv(jsc.conv) void;
...
`;
    if (memoryCost) {
      exports2.set("memoryCost", symbolName(typeName, "memoryCost"));
      output += `
  ...
  `;
    ...
    if (estimatedSize) {
      exports2.set("estimatedSize", symbolName(typeName, "estimatedSize"));
      output += `
    ...
    if (hasPendingActivity) {
      exports2.set("hasPendingActivity", symbolName(typeName, "hasPendingActivity"));
      output += `
    ...
    if (finalize) {
      exports2.set("finalize", classSymbolName(typeName, "finalize"));
      output += `
    ...
    if (construct && !noConstructor) {
      exports2.set("construct", classSymbolName(typeName, "construct"));
    ...
    if (call) {
      exports2.set("call", classSymbolName(typeName, "call"));
      output += `
    ...
    if (getInternalProperties) {
      exports2.set("getInternalProperties", classSymbolName(typeName, "getInternalProperties"));
      output += `
    ...
    {
      const exportNames = (name) => zigExportName(exports2, (name2) => protoSymbolName(typeName, name2), proto[name]);
      for (const name in proto) {
      ...
      `;
        ...
        if (names.fn) {
          if (names.DOMJIT) {
            ...
            output += `
          pub fn ${names.DOMJIT}(thisValue: *${typeName}, globalObject: *jsc.JSGlobalObject, ${args.map(ZigDOMJITArgTypeDefinition).join(", ")}) callconv(jsc.conv) jsc.JSValue {
    ...
    {
      const exportNames = (name) => zigExportName(exports2, (name2) => classSymbolName(typeName, name2), klass[name]);
      for (const name in klass) {
        ...
        if (names.fn) {
          if (DOMJIT) {
            const { args, returns } = DOMJIT;
            output += `
          pub fn ${names.DOMJIT}(globalObject: *jsc.JSGlobalObject, thisValue: jsc.JSValue, ${args.map(ZigDOMJITArgTypeDefinition).join(", ")}) callconv(jsc.conv) jsc.JSValue {
    ...
    if (structuredClone) {
      exports2.set("structuredClone", symbolName(typeName, "onStructuredCloneSerialize"));
      output += `
      ...
      `;
      if (typeof structuredClone === "object" && structuredClone.transferable) {
        exports2.set("structuredClone_transferable", symbolName(typeName, "onStructuredCloneTransfer"));
        output += `
        pub fn ${exports2.get("structuredClone_transferable")}(thisValue: *${typeName}, globalObject: *jsc.JSGlobalObject, ctx: *anyopaque, write: WriteBytesFn) callconv(jsc.conv) void {
      ...
      exports2.set("structuredCloneDeserialize", symbolName(typeName, "onStructuredCloneDeserialize"));
      output += `
  ...
  comptime {
${[...exports2.values()].map((name) => `      @export(&JavaScriptCoreBindings.${name}, .{ .name = "${name}" });`).join("\n")}
    }`;
  }
  return `
  ...
  return `
          m_${className(typeName)}.initLater(
              [](LazyClassStructure::Initializer& init) {
                 ...
                 ${noConstructor ? "" : `init.setConstructor(WebCore::${className(
    typeName
...
var GENERATED_CLASSES_IMPL_HEADER_PRE = `
// GENERATED CODE - DO NOT MODIFY BY HAND
...
`;
var GENERATED_CLASSES_IMPL_HEADER_POST = `
namespace WebCore {
...
`;
var GENERATED_CLASSES_IMPL_FOOTER = `
...
function jsInheritsCppImpl() {
  return `
${js_classes_default.map((v) => v[1]).filter((v) => v?.length > 0).map((v, i) => `#include "${v}"`).join("\n")}
...
JSC_DEFINE_HOST_FUNCTION(Zig::jsFunctionInherits, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    switch (id) {
${js_classes_default.map((v) => v[0]).map((v, i) => `    case ${i}: return JSValue::encode(jsBoolean(dynamicDowncast<WebCore::JS${v}>(cell) != nullptr));`).join("\n")}
    }
...
function visitLazyClasses(classes2) {
  return `

template<typename Visitor>
void GlobalObject::visitGeneratedLazyClasses(GlobalObject *thisObject, Visitor& visitor)
{
      ${classes2.map((a) => `thisObject->m_${className(a.name)}.visit(visitor);`).join("\n      ")}
}

  `.trim();
}
var ZIG_GENERATED_CLASSES_HEADER = `
...
var classes = [];
{
  ...
  for (const file of files) {
    ...
    if (!(result?.default?.length ?? 0)) {
      errors.push(
        new TypeError(
          `Missing classes in "${path2.relative(process.cwd(), filepath)}". Expected \`export default [ define(...) ] satisfies Array<ClassDefinition>\` but got ${Bun.inspect(result).slice(0, 100) + "..."} `
        )
      );
      continue;
    }
...
function writeCppSerializers() {
  var output = ``;
  var structuredClonable = classes.filter((a) => a.structuredClone).sort((a, b) => a.structuredClone.tag < b.structuredClone.tag);
...
function getPropertySignatureWithComment(propName, propDef, classDef) {
  ...
  if ("fn" in propDef || "builtin" in propDef) {
    ...
    let args = Array(length).fill(0).map((_, i) => `arg${i}?: unknown`).concat(...length > 0 ? ["...args: unknown[]"] : []).join(", ");
    let returnType = "unknown";
    if (propDef.async) {
      returnType = "Promise<unknown>";
    }
    signature = `${tsPropName}(${args}): ${returnType};`;
    isMethod = true;
    if ("fn" in propDef) {
      commentLines.push(
...
function generateBuiltinTypes(classes2) {
  const typeDeclarations = [];
  for (const classDef of classes2) {
    if (classDef.zigOnly) continue;
