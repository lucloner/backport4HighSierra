...
// ../bun/src/jsc/bindings/ErrorCode.ts
var errors = [
...
var ErrorCode_default = errors;

// ../bun/src/codegen/helpers.ts
...
var extra_count = ErrorCode_default.map((x) => x.slice(3)).filter((x) => x.length > 0).reduce((ac, cv) => ac + cv.length, 0);
var count = ErrorCode_default.length + extra_count;
if (count > 1 << 16) {
...
`;
var i = 0;
for (let [code, constructor, name, ...other_constructors] of ErrorCode_default) {
...
`;
var builtindtsPath = path2.join(import.meta.dirname, "..", "..", "src", "js", "builtins.d.ts");
var builtindts = fs2.readFileSync(builtindtsPath, "utf-8");
var dts = `
// Generated by: src/codegen/generate-node-errors.ts
// Input:        src/jsc/bindings/ErrorCode.ts
...
`;
for (const [code, constructor, name, ...other_constructors] of ErrorCode_default) {
  const hasExistingOverride = builtindts.includes(`declare function $${code}`);
  if (hasExistingOverride) {
    continue;
  }
  const namedError = name && name !== constructor.name ? `${constructor.name} & { name: "${name}", code: "${code}" }` : `${constructor.name} & { code: "${code}" }`;
  dts += `
/**
...
writeIfNotChanged(path2.join(outputDir, "ErrorCode+List.h"), enumHeader);
writeIfNotChanged(path2.join(outputDir, "ErrorCode+Data.h"), listHeader);
writeIfNotChanged(path2.join(outputDir, "ErrorCode.zig"), zig);
writeIfNotChanged(path2.join(outputDir, "ErrorCode.d.ts"), dts);
